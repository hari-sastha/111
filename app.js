...
function getDbmsSteps() {
	return [
		'1) Create a file for your experiment: .sql for exp1-3, .js for exp4, and .py for exp5-7.',
		'2) Type the snippet prefix (e.g., dbms-exp1) and press Tab to paste the code inside and save.',
		'',
		'--- Experiments 1 to 4 (SQL / MongoDB) ---',
		'You can just use an online compiler if you prefer to avoid setting up a local database.',
		'- Try online compilers like OneCompiler or Programiz to test the queries instantly.',
		'',
		'--- Experiments 5 to 7 (Python) ---',
		'These require Python and background databases. Using Docker Desktop is the easiest way to run them.',
		'- Download Python: https://www.python.org/downloads/',
		'- Download Docker Desktop: https://www.docker.com/products/docker-desktop/ (Install and open it, wait until "Engine running")',
		'',
		'>>> For Exp5 (Cassandra + Python):',
		'- Start Cassandra in terminal: docker run --name cassandra -p 9042:9042 -d cassandra:latest',
		'- Install Python driver: pip install cassandra-driver',
		'- Run your file: python yourfile.py',
		'',
		'>>> For Exp6 (Redis + Python):',
		'- Start Redis in terminal: docker run --name redis -p 6379:6379 -d redis:latest',
		'- Install Python driver: pip install redis',
		'- Run your file: python yourfile.py',
		'',
		'>>> For Exp7 (Flask API):',
		'- Doesn\'t need Docker (uses local SQLite automatically).',
		'- Install dependencies: pip install flask flask-sqlalchemy flask-bcrypt flask-cors',
		'- Run your file: python yourfile.py',
		'- Open http://127.0.0.1:5000 in your browser or test routes with Postman.'
	];
}
